...
export const RegisterPage = () => {
    ...
    return (
        <div>
            ...
            <p className="-mt-3 text-sm text-gray-500">Already have an account?<Link to={{pathname: "/login"}} className="ml-1 cursor-pointer font-medium text-indigo-600 hover:underline">Log In</Link></p>
            ...
            <form onSubmit={register} className="flex flex-col gap-5">
                ...
                <label htmlFor="name" className="-mb-3 block text-sm font-medium text-gray-700">Name</label>
                <input onChange={(e) => inputName(e)} name="name" className={`rounded-lg border ${!isValid && "border-red-400 border-2"} border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500`} />

                {!isValid && <p className="-mb-3 text-red-400 mt-0 pt-0 block text-sm font-medium">Wrong creadentials</p>}
